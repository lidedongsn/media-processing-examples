...
AVFrame* H264Decoder::Decode(uint8_t* data, uint32_t len,
                             uint32_t* decode_len) {
  AVFrame* out_frame;
  int ret;
  ...
  *decode_len = len;
  ...
  ret = av_parser_parse2(m_parser, m_context, &m_pkt->data, &m_pkt->size,
                         (const uint8_t*)data, (int)len, AV_NOPTS_VALUE,
                         AV_NOPTS_VALUE, 0);
  if (ret < 0) {
...
void H264Decoder::Dump(FILE* f) {
  if (m_frame) {
    ...
    for (i = 0; i < pFrame->height / 2; i++) {
      fwrite(pFrame->data[1] + pFrame->linesize[1] * i, 1, pFrame->width / 2,
             f);
    }
    for (i = 0; i < pFrame->height / 2; i++) {
      fwrite(pFrame->data[2] + pFrame->linesize[2] * i, 1, pFrame->width / 2,
             f);
    }
  }
}
