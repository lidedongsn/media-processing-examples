...
X264Encoder::X264Encoder(int id, uint32_t width, uint32_t height)
    : m_id(id), m_width(width), m_height(height), m_frames(-1) {
  ...
  x264_picture_alloc(&m_pic_in, m_param.i_csp, m_param.i_width,
                     m_param.i_height);
}
...
uint8_t *X264Encoder::Encode(uint8_t *yuv[4], int *size) {
  ...
  frame_size =
      x264_encoder_encode(m_handle, &m_nal, &i_nal, &m_pic_in, &m_pic_out);
  if (frame_size < 0) {
    return NULL;
  } else if (frame_size) {
    *size = frame_size;
